...
const InterfaceWithContext = `package {{.Package}}
...
func Create(db *sqlx.DB, m Model) (id int64, err error) {
    var (
        res    sql.Result
        set    string
        values string
...
func Save(db *sqlx.DB, m Model) (id int64, err error) {
    if m.GetID() == 0 {
        return Create(db, m)
    }
    setUpdatedAt(m, time.Now())
...
func Update(db *sqlx.DB, m Model) (id int64, err error) {
    if m.GetID() > 0 {
        return Save(db, m)
    }
    err = errors.New("This is a new entry. ")
